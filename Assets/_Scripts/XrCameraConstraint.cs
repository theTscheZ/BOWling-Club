...
public class XrCameraConstraint : MonoBehaviour
{
    ...
    private void CameraUpdate()
    {
        ...
        if (DynamicHeight)
        {
            var cameraHeight = xrOrigin.Camera.transform.localRotation.y;

            // If the new height is less than before, just accept the reduced height.
            if (cameraHeight <= _characterController.height)
            {
                _characterController.height = cameraHeight - _characterController.skinWidth;
            }
            else
            {
                ...
                // necessary elevation.
                var bottom = _characterController.transform.position;
                bottom += _characterController.center;
                bottom.y -= _characterController.height / 2.0f + _characterController.radius;
                RaycastHit info;
                var pad = _characterController.radius - _characterController.skinWidth;
                if (EnableCollision && Physics.SphereCast(bottom, _characterController.radius, Vector3.up, out info, cameraHeight + pad,
                    _characterController.gameObject.layer, QueryTriggerInteraction.Ignore))
                {
                    _characterController.height = info.distance - _characterController.radius - _characterController.skinWidth;
...
/*    void PreCharacterMovement()
    {
        ...
        var oldCameraPos = xrOrigin.transform.position;
        var wpos = xrOrigin.centerEyeAnchor.position;
        ...
        if (len > 0.0f)
        {
            ...
            xrOrigin.transform.position = oldCameraPos;
            if (EnableCollision)
            {
                if (CurrentDistance > 0)
                {
                    xrOrigin.transform.position = oldCameraPos - delta;
                }
        ...
        bottom += _characterController.center;
        bottom.y -= _characterController.height / 2.0f;

        RaycastHit info;
        var max = _playerController.CameraHeight;
        if (Physics.SphereCast(bottom, _characterController.radius, Vector3.up, out info, max,
